	...
	$.ajax({
		...
		success:function(result){
			...
			for (var i = 0; i < result.length; i++) {
				...
				$(".pill-nav").append("<a onClick=\"statusChange(this)\" href=\"/client/notice/listz?division_id="+division.division_id+"\">"+division.dname+"</a>")
			}
		}
	});
}
...
function statusChange(statusItem){
	var dnameText =$(statusItem).text();
	$(".dname").text(dnameText);
	console.log(dnameText);
}
</script>
</head>
<body><!-- id="href 이름" -->
	...
	<table>
		...
		<tr>
			...
			<%if(i>pager.getTotalPage()) break; %>
				<a <%if(pager.getCurrentPage()==i){ %>class="pageNum"<%} %> href="/client/notice/listz?currentPage=<%=i%>">[<%=i %>]</a>
			<%} %>
			</td>
		</tr>
		<tr>
